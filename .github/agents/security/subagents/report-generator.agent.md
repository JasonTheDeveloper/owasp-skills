...
description: "Collates verified OWASP skill assessment findings and generates a comprehensive vulnerability report written to docs/reports/"
tools:
...
Report filename pattern (audit): `vulnerability-report-{{YYYY-MM-DD}}-{{NNN}}.md`

Report filename pattern (diff): `vulnerability-report-diff-{{YYYY-MM-DD}}-{{NNN}}.md`

Report filename pattern (plan): `plan-risk-assessment-{{YYYY-MM-DD}}-{{NNN}}.md`
...
* Change the H1 title to: `# OWASP Vulnerability Assessment Report — Changed Files Only`
...
1. Create the `docs/reports/` directory if it does not exist.
...
1. Select the filename prefix based on mode:
   * When mode is `audit`: search for `vulnerability-report-{REPORT_DATE}-*.md`.
   * When mode is `diff`: search for `vulnerability-report-diff-{REPORT_DATE}-*.md`.
   * When mode is `plan`: search for `plan-risk-assessment-{REPORT_DATE}-*.md`.
2. Search `docs/reports/` for existing files matching the selected pattern where `{REPORT_DATE}` is the provided report date.
...
* When mode is `diff`:
  ...
  2. Use the diff mode title: `# OWASP Vulnerability Assessment Report — Changed Files Only`.
...
1. Select the report format and filename pattern based on mode:
   * When mode is `audit`: assemble the report following VULN_REPORT_V1 and write to `docs/reports/vulnerability-report-{REPORT_DATE}-{NNN}.md`.
   * When mode is `diff`: assemble the report following VULN_REPORT_V1 with diff mode qualifiers and write to `docs/reports/vulnerability-report-diff-{REPORT_DATE}-{NNN}.md`.
   * When mode is `plan`: assemble the report following PLAN_REPORT_V1 and write to `docs/reports/plan-risk-assessment-{REPORT_DATE}-{NNN}.md`.
...
* Mode that determined the report format.
* Status of the generation: complete or incomplete.
* Total findings count and severity breakdown.
* Clarifying questions, if any inputs are ambiguous or missing.
