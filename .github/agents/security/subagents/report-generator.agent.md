...
description: "Collates verified security skill assessment findings and generates a comprehensive vulnerability report written to docs/reports/ - Brought to you by microsoft/hve-core"
tools:
...
Report path pattern (audit): `docs/reports/security/{{YYYY-MM-DD}}/security-report-{{NNN}}.md`

Report path pattern (diff): `docs/reports/security/{{YYYY-MM-DD}}/security-report-diff-{{NNN}}.md`

Report path pattern (plan): `docs/reports/security/{{YYYY-MM-DD}}/plan-risk-assessment-{{NNN}}.md`
...
1. Create the `docs/reports/security` directory if it does not exist.
...
2. Search `docs/reports/security/{REPORT_DATE}` for existing files matching the selected pattern where `{REPORT_DATE}` is the provided report date.
...
1. Select the report format and filename pattern based on mode:
   * When mode is `audit`: assemble the report following VULN_REPORT_V1 and write to `docs/reports/security/{REPORT_DATE}/security-report-{NNN}.md`.
   * When mode is `diff`: assemble the report following VULN_REPORT_V1 with diff mode qualifiers and write to `docs/reports/security/{REPORT_DATE}/security-report-diff-{NNN}.md`.
   * When mode is `plan`: assemble the report following PLAN_REPORT_V1 and write to `docs/reports/security/{REPORT_DATE}/plan-risk-assessment-{NNN}.md`.
